...
# This model is to store the data from external resource json files
...
# This part with additional models is apart from the course.
class ThreeStepCar(models.Model):
    id = models.AutoField(primary_key=True)
    state = models.CharField(null=False, max_length=200)  #  
    city_code = models.CharField(null=False, max_length=15)
    phone = models.CharField(null=False, max_length=30)

    def __str__(self):
        return self.state + " "+ self.city_code    
...
class MyTuningModel(models.Model):
    id = models.AutoField(primary_key=True)
